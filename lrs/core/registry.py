...
class ToolRegistry:
    ...
    def register(self, tool: ToolLens, alternatives: Optional[List[str]] = None):
        """
    ...
    def _schemas_compatible(self, schema_a: Dict[str, Any], schema_b: Dict[str, Any]) -> bool:
        """
        ...
        if type_a == "object":
            ...
            # schema_b must provide all fields required by schema_a
            if not req_a.issubset(req_b):
                return False

        return True

    def update_statistics(self, tool_name: str, success: bool, prediction_error: float):
        """
        ...
        if n > 1:
            ...
            stats["error_variance"] = old_var + (prediction_error - old_avg) * (
                prediction_error - new_avg
            )
